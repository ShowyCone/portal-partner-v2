...
const ServiceCard: React.FC<Props> = ({ service, index = 0, variant = 'default' }) => {
    ...
    <Link href={`/service/${service.id}`} className="block">
      ...
      >
        <div className="relative h-48 rounded-t-lg overflow-hidden">
          ...
          >
            <FaRegHeart className="group-hover:scale-125 transition-all duration-300" size={22} />
          </button>
        </div>
        ...
        <div className="p-4 flex flex-col flex-grow text-gray-800">
          <div className="flex justify-between items-start gap-2">
            <div className="relative group flex-1 min-w-0">
              <h3 className="font-bold text-base text-rwa truncate">{service.title}</h3>
          ...
          <div className={isSuggested ? 'flex flex-col items-center gap-1 mt-2' : 'flex items-center gap-2 mt-2'}>
            <p className="font-bold text-gray-900 text-base">${service.price}</p>
            <span className="bg-rwa/10 text-rwa text-xs font-semibold px-2 py-0.5 rounded-full">{service.tag}</span>
          </div>
          ...
          {!isSuggested && (
            <div className="border-t border-gray-200 mt-4 pt-3 flex justify-between items-center text-sm text-gray-500">
              {partner ? (
                <Link href={`/partner/${partner.id}`} className="hover:underline hover:text-rwa">
                  {partnerName}
                </Link>
              ) : (
